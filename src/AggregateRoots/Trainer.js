...
module.exports = function(core, uuid, invariant) {
    return class Trainer extends core.AggregateRootBase {
        constructor() {
            super();
            var _password;
            var _loggedIn;
        ...
        commandHandlers() {
            return {
                'hireTrainer': function (cmd) {
                    this.raiseEvent(new core.GesEvent('trainerHired',{id: uuid.v4(), credentials:cmd.credentials, contact:cmd.contact, address:cmd.address, dob:cmd.dob}));
                },
                'loginTrainer': function (cmd) {
                    expectNotLoggedIn();
                    expectCorrectPassword(cmd.password);
                    var token = createToken();
                    this.raiseEvent(new core.GesEvent('trainerLoggedIn',{id:this._id, userName:cmd.userName, token:token, created:new Date()}));
                },
